...
def train(
    ...
    for itr in range(1, args.niters + 1):
        ...
        with torch.no_grad():
            pred = model(full_data)  # Adjust according to your model's output
            mse = F.mse_loss(pred, full_data).item()
            mses.append(mse)

        if len(regularization_coeffs) > 0:
            # Regularize on the last timepoint
    ...
    # Plot metrics
    plt.figure()
    plt.plot(range(1, args.niters + 1), losses, label="Loss")
    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    plt.title("Loss vs Iteration")
    plt.legend()
    plt.savefig(os.path.join(args.save, "loss_vs_iteration.png"))

    plt.figure()
    plt.plot(range(1, args.niters + 1), mses, label="MSE", color="orange")
    plt.xlabel("Iteration")
    plt.ylabel("MSE")
    plt.title("MSE vs Iteration")
    plt.legend()
    plt.savefig(os.path.join(args.save, "mse_vs_iteration.png"))
